...
const COPY = {
  en: {
    menu: 'Menu', promos: 'Promos', rewards: 'Rewards', hours: 'HOURS', social: 'SOCIAL',
    pickup: 'PICKUP', delivery: 'DELIVERY', allItems: 'ALL ITEMS', customize: 'Customize',
    location: 'YOUR LOCATION', directions: 'GET DIRECTIONS', storeDetails: 'STORE DETAILS',
    pickupAvailable: 'Pickup Available', deliveryAvailable: 'Delivery Available',
    deliveryFee: 'Delivery Fee', deliveryRadius: 'Delivery Radius', minimumOrder: 'Minimum Order',
    required: 'Required', optional: 'Optional', quantity: 'Quantity', addToCart: 'ADD TO CART',
    cart: 'View Cart', checkout: 'CHECKOUT', subtotal: 'SUBTOTAL', discount: 'DISCOUNT',
    total: 'TOTAL', empty: 'Your cart is empty.', basePrice: 'Base Price', finalPrice: 'Final Price',
    soldOut: 'Sold Out', openNow: 'Open Now', closedNow: 'Closed Now', closes: 'Closes',
    opens: 'Opens', remove: 'Remove', apply: 'Apply', usePromo: 'Use Promo', promoCode: 'Promo code',
    promoApplied: 'Promo applied.', promoNotFound: 'Promo not found.', limitedOffer: 'LIMITED TIME OFFER',
    nextOrder: 'YOUR NEXT ORDER', applyCode: 'APPLY CODE', pointsAvailable: 'POINTS AVAILABLE',
    nextReward: 'Next Reward', viewRewards: 'VIEW REWARDS', items: 'ITEMS',
    noPhone: 'Phone number not added yet', noItems: 'No menu items yet.',
    noItemsSub: 'Add items in the ORDA builder and they will show here.', orderType: 'Order Type',
    working: 'WORKING...', directTagline: 'Order direct. Fresh food.', followUs: 'Follow Us',
    videos: 'VIDEOS', playVideo: 'Play Video'
  },
  es: {
    menu: 'Menú', promos: 'Promos', rewards: 'Recompensas', hours: 'HORARIO', social: 'REDES',
    pickup: 'RECOGER', delivery: 'ENTREGA', allItems: 'TODO', customize: 'Personalizar',
    location: 'TU UBICACIÓN', directions: 'CÓMO LLEGAR', storeDetails: 'DETALLES',
    pickupAvailable: 'Pickup Disponible', deliveryAvailable: 'Entrega Disponible',
    deliveryFee: 'Costo de Entrega', deliveryRadius: 'Radio de Entrega', minimumOrder: 'Orden Mínima',
    required: 'Requerido', optional: 'Opcional', quantity: 'Cantidad', addToCart: 'AGREGAR',
    cart: 'Ver Carrito', checkout: 'PAGAR', subtotal: 'SUBTOTAL', discount: 'DESCUENTO',
    total: 'TOTAL', empty: 'Tu carrito está vacío.', basePrice: 'Precio Base', finalPrice: 'Precio Final',
    soldOut: 'Agotado', openNow: 'Abierto', closedNow: 'Cerrado', closes: 'Cierra',
    opens: 'Abre', remove: 'Quitar', apply: 'Aplicar', usePromo: 'Usar Promo', promoCode: 'Código promo',
    promoApplied: 'Promo aplicada.', promoNotFound: 'Promo no encontrada.', limitedOffer: 'OFERTA POR TIEMPO LIMITADO',
    nextOrder: 'TU PRÓXIMA ORDEN', applyCode: 'APLICAR CÓDIGO', pointsAvailable: 'PUNTOS DISPONIBLES',
    nextReward: 'Próxima Recompensa', viewRewards: 'VER RECOMPENSAS', items: 'ARTÍCULOS',
    noPhone: 'Número no agregado todavía', noItems: 'No hay artículos todavía.',
    noItemsSub: 'Agrega artículos en ORDA builder y aparecerán aquí.', orderType: 'Tipo de Orden',
    working: 'TRABAJANDO...', directTagline: 'Ordena directo. Comida fresca.', followUs: 'Síguenos',
    videos: 'VIDEOS', playVideo: 'Ver Video'
  },
};

const OWNER_TRANSLATIONS: Record<Lang, Record<string, string>> = {
  en: { pickup: 'Pickup', delivery: 'Delivery', subtotal: 'Subtotal', deliveryFee: 'Delivery Fee', discount: 'Discount', total: 'Total', promo: 'Promo', noSelections: 'No options selected' },
  es: { pickup: 'Recoger', delivery: 'Entrega', subtotal: 'Subtotal', deliveryFee: 'Costo de Entrega', discount: 'Descuento', total: 'Total', promo: 'Promo', noSelections: 'Sin opciones' },
};
...
function categoryFromName(categoryName?: string | null, itemName?: string | null) {
  const text = cleanKey(`${categoryName || ''} ${itemName || ''}`);
  ...
  if (text.includes('mexican') || text.includes('burrito') || text.includes('quesadilla') || text.includes('nacho')) return 'mexican';
  if (text.includes('seafood') || text.includes('shrimp') || text.includes('fish') || text.includes('crab')) return 'seafood';
  if (text.includes('bbq') || text.includes('barbecue') || text.includes('rib') || text.includes('brisket')) return 'bbq';
  ...
  if (text.includes('hot_dog') || text.includes('hotdog')) return 'hot_dog';
  if (text.includes('fries') || text.includes('fry')) return 'fries';
  if (text.includes('breakfast')) return 'breakfast';
  if (text.includes('dessert') || text.includes('cake')) return 'desserts';
  if (text.includes('drink') || text.includes('soda') || text.includes('juice') || text.includes('water')) return 'drinks';
  if (text.includes('side')) return 'sides';
  if (text.includes('combo')) return 'combos';
  return 'universal';
}

function fallbackImage(categoryName?: string | null, itemName?: string | null) {
  return publicStorageUrl(`${categoryFromName(categoryName, itemName)}/1.jpg`);
}

function fallbackForImageElement(event: SyntheticEvent<HTMLImageElement, Event>, categoryName?: string, itemName?: string) {
  event.currentTarget.onerror = null;
  event.currentTarget.src = fallbackImage(categoryName, itemName || FALLBACK_FOOD);
}
...
function formatHour(value: string) {
  const minutes = parseHourMinute(value);
...
function normalizeHours(raw: any): StoreHour[] {
  ...
  if (Array.isArray(value)) {
    return DEFAULT_HOURS.map((fallback, index) => {
      const found = value[index] || value.find((hour: any) => String(hour?.day || '').toLowerCase() === fallback.day.toLowerCase()) || {};
      return { day: found.day || fallback.day, open: String(found.open || found.open_time || fallback.open), close: String(found.close || found.close_time || fallback.close), closed: Boolean(found.closed || found.is_closed || found.open === false || found.isOpen === false) };
    });
  }
  if (typeof value === 'object') {
    return DAYS.map(([key, label]) => {
      const found = value[key] || value[label] || value[label.toLowerCase()] || {};
      return { day: label, open: String(found.open || found.open_time || DEFAULT_HOURS.find((hour) => hour.day === label)?.open || '11:00 AM'), close: String(found.close || found.close_time || DEFAULT_HOURS.find((hour) => hour.day === label)?.close || '10:00 PM'), closed: Boolean(found.closed || found.is_closed || found.open === false || found.isOpen === false) };
    });
  }
  return DEFAULT_HOURS;
}

function getOpenStatus(hoursRaw: any, copy: typeof COPY.en): OpenStatus {
  const hours = normalizeHours(hoursRaw);
  const now = new Date();
  const dayIndex = now.getDay() === 0 ? 6 : now.getDay() - 1;
  const today = hours[dayIndex] || DEFAULT_HOURS[dayIndex];
  const current = now.getHours() * 60 + now.getMinutes();
  const openMinutes = parseHourMinute(today.open);
  const closeMinutes = parseHourMinute(today.close);
  if (!today.closed && openMinutes !== null && closeMinutes !== null) {
    const overnight = closeMinutes < openMinutes;
    const open = overnight ? current >= openMinutes || current <= closeMinutes : current >= openMinutes && current <= closeMinutes;
    if (open) return { open: true, text: copy.openNow, nextText: `${copy.closes} ${formatHour(today.close)}` };
  }
  const nextOpen = hours.find((row, offset) => offset === 0 ? !row.closed && current < (parseHourMinute(row.open) || 0) : !row.closed);
  return { open: false, text: copy.closedNow, nextText: `${copy.opens} ${formatHour(nextOpen?.open || today.open)}` };
}
...
function isActiveRecord(row: any) {
  ...
  const endValue = row.ends_at || row.expires_at;
  if (endValue) {
...
function isLiveMenuItem(row: ItemRow) {
  const availability = String(row.availability || '').toLowerCase().trim();
  if (row.is_available === false) return false;
  if (['deleted', 'delete', 'removed', 'remove', 'hidden', 'inactive', 'archived', 'draft', 'sold_out'].includes(availability)) return false;
  if (!String(row.name || '').trim()) return false;
  return true;
}

function isVideoMenuRow(row: ItemRow) {
  const video = resolveStorageUrl(row.video_file || row.item_video || row.menu_video || row.video_url || '');
  return String(row.availability || '').toLowerCase().trim() === VIDEO_MENU_AVAILABILITY || String(row.media_type || '').toLowerCase().includes('video') || isVideoUrl(video);
}
...
function promoValueText(promo: PromoRow) {
  ...
  if (!value && !type.includes('free')) return 'MEMBER PERK';
  if (type.includes('percent')) return `${value}% OFF`;
  if (type.includes('free')) return 'FREE ITEM';
  return `${money(value)} OFF`;
}

function promoDiscountAmount(promo: PromoRow | null, subtotal: number) {
  if (!promo) return 0;
  const minimum = Number(promo.minimum_order ?? promo.min_order ?? 0);
  if (minimum > 0 && subtotal < minimum) return 0;
  const type = String(promo.discount_type || '').toLowerCase();
  const value = Number(promo.discount_value ?? promo.value ?? 0);
  if (!value) return 0;
  if (type.includes('percent')) return Math.min(subtotal, subtotal * (value / 100));
...
async function fetchOptionalRows<T>(tableNames: string[], restaurantId: string) {
  for (const table of tableNames) {
    const { data, error } = await supabase.from(table).select('*').eq('restaurant_id', restaurantId).limit(25);
    if (!error && Array.isArray(data)) return data as T[];
  }
  return [] as T[];
}

function choices(names: Array<string | [string, number]>): Choice[] {
  return names.map((entry) => {
    const name = Array.isArray(entry) ? entry[0] : entry;
    const priceDelta = Array.isArray(entry) ? entry[1] : 0;
    return { id: cleanKey(name), name, priceDelta };
  });
}

function isVideoMenuItemFromItem(item: Item) {
  return item.isVideoMenu || cleanKey(item.categoryName).includes('video_menu') || isVideoUrl(resolveStorageUrl(item.videoUrl));
}

function defaultGroups(item: Item): Group[] {
  const key = cleanKey(`${item.categoryName} ${item.name}`);
  if (key.includes('burger') || key.includes('sandwich') || key.includes('hot_dog') || key.includes('hotdog')) {
    return [
      { id: `${item.id}_remove`, name: 'Remove Items', required: false, mode: 'multiple', choices: choices(['No Pickles', 'No Onions', 'No Tomato', 'No Lettuce', 'No Sauce']) },
      { id: `${item.id}_addons`, name: 'Add Ons', required: false, mode: 'multiple', choices: choices([['Add Cheese', 1], ['Add Bacon', 2], ['Extra Patty', 4], ['Avocado', 2]]) },
    ];
  }
  if (key.includes('taco') || key.includes('mexican') || key.includes('burrito') || key.includes('quesadilla') || key.includes('nacho')) {
    return [
      { id: `${item.id}_protein`, name: 'Protein', required: false, mode: 'single', choices: choices(['Asada', 'Chicken', 'Al Pastor', 'Carnitas', 'Birria', 'Shrimp']) },
      { id: `${item.id}_toppings`, name: 'Toppings', required: false, mode: 'multiple', choices: choices(['Sour Cream', 'Cilantro', 'Onions', 'Cheese', 'Lettuce', 'Pico de Gallo', ['Guacamole', 2], ['Queso', 1.5]]) },
      { id: `${item.id}_salsa`, name: 'Salsa', required: false, mode: 'single', choices: choices(['Mild Salsa', 'Medium Salsa', 'Hot Salsa', 'Salsa Verde', 'Salsa Roja', 'No Salsa']) },
    ];
  }
  if (key.includes('wing')) {
    return [
      { id: `${item.id}_flavor`, name: 'Wing Flavor', required: true, mode: 'single', choices: choices(['Fried', 'Buffalo', 'BBQ', 'Lemon Pepper', 'Garlic Parmesan', 'Honey Hot', 'Hot']) },
      { id: `${item.id}_wing_style`, name: 'Wing Style', required: false, mode: 'multiple', choices: choices(['Extra Crispy', 'Sauce On Side', ['All Flats', 2], ['All Drums', 1]]) },
      { id: `${item.id}_dip`, name: 'Dipping Sauce', required: false, mode: 'multiple', choices: choices(['Ranch', 'Blue Cheese', ['Extra Ranch', 0.75], ['Extra Blue Cheese', 0.75]]) },
    ];
  }
  return [{ id: `${item.id}_special`, name: 'Special Instructions', required: false, mode: 'multiple', choices: choices([['Extra Sauce', 0.75], 'No Sauce', ['Extra Cheese', 1.5], 'No Onions', 'No Vegetables']) }];
}

function buildOwnerSummary(cart: CartLine[], values: { orderType: OrderType; subtotal: number; deliveryFee: number; discount: number; total: number; promo?: string | null }, lang: Lang) {
  const c = OWNER_TRANSLATIONS[lang];
  const lines: string[] = [];
  lines.push(`${lang === 'es' ? 'Tipo de orden' : 'Order Type'}: ${values.orderType === 'delivery' ? c.delivery : c.pickup}`);
  lines.push('');
  cart.forEach((line, index) => {
    lines.push(`${index + 1}. ${line.quantity}x ${line.name} — ${money(line.total)}`);
    lines.push(`   Item ID: ${line.itemId}`);
    if (line.imageUrl) lines.push(`   Image: ${line.imageUrl}`);
    if (line.videoUrl) lines.push(`   Video: ${line.videoUrl}`);
    if (line.selections.length) line.selections.forEach((selection) => lines.push(`   ${selection.groupName}: ${selection.choiceNames.join(', ')}`));
    else lines.push(`   ${c.noSelections}`);
  });
  lines.push('');
  lines.push(`${c.subtotal}: ${money(values.subtotal)}`);
  if (values.deliveryFee > 0) lines.push(`${c.deliveryFee}: ${money(values.deliveryFee)}`);
  if (values.discount > 0) lines.push(`${c.discount}: -${money(values.discount)}`);
  if (values.promo) lines.push(`${c.promo}: ${values.promo}`);
  lines.push(`${c.total}: ${money(values.total)}`);
  return lines.join('\n');
}

function normalizeSocialUrl(kind: SocialKind, value?: string | null) {
  const raw = String(value || '').trim();
  if (!raw) return '';
  ...
  if (kind === 'tiktok') return `https://www.tiktok.com/@${clean}`;
  return clean.startsWith('@') ? `https://youtube.com/${clean}` : `https://youtube.com/@${clean}`;
}

function getSocialLinks(restaurant: RestaurantRow | null) {
  if (!restaurant) return [];
  const links = [
    { key: 'instagram' as SocialKind, label: 'Instagram', icon: 'IG', url: normalizeSocialUrl('instagram', restaurant.instagram_url || restaurant.instagram) },
    { key: 'facebook' as SocialKind, label: 'Facebook', icon: 'f', url: normalizeSocialUrl('facebook', restaurant.facebook_url || restaurant.facebook) },
    { key: 'tiktok' as SocialKind, label: 'TikTok', icon: '♪', url: normalizeSocialUrl('tiktok', restaurant.tiktok_url || restaurant.tiktok) },
    { key: 'youtube' as SocialKind, label: 'YouTube', icon: '▶', url: normalizeSocialUrl('youtube', restaurant.youtube_url || restaurant.youtube) },
  ];
  return links.filter((link) => link.url);
}

function VideoPosterImage({ poster, title, categoryName }: { poster: string; title: string; categoryName?: string }) {
  const safePoster = poster || fallbackImage(categoryName, title);
  return <img src={safePoster} alt={title} loading="lazy" decoding="async" onError={(event) => fallbackForImageElement(event, categoryName, title)} />;
}
...
function SmoothVideo({ src, poster, className = '', limitSeconds, controls = false, playWhenVisible = true }: { src: string; poster?: string; className?: string; limitSeconds?: number; controls?: boolean; playWhenVisible?: boolean }) {
  ...
  return (
    <video
      ...
      onLoadedMetadata={(event) => playVideo(event.currentTarget)}
      onLoadedData={(event) => playVideo(event.currentTarget)}
      onCanPlay={(event) => playVideo(event.currentTarget)}
      onPause={(event) => { if (!controls && playWhenVisible) window.setTimeout(() => playVideo(event.currentTarget), 200); }}
      onEnded={(event) => { event.currentTarget.currentTime = 0; playVideo(event.currentTarget); }}
      onTimeUpdate={(event) => {
...
function VideoCover({ src, poster, title, categoryName }: { src?: string; poster?: string; title: string; categoryName?: string }) {
  const safePoster = poster || fallbackImage(categoryName, title);
  if (src && isVideoUrl(src)) {
    return (
      <div className="videoCoverBox" aria-label={title}>
        <video src={src} poster={safePoster || undefined} muted playsInline preload="metadata" onLoadedMetadata={(event) => { try { event.currentTarget.currentTime = 0.8; } catch {} }} />
        <img src={safePoster} alt={title} loading="lazy" decoding="async" onError={(event) => { event.currentTarget.style.display = 'none'; }} />
        <div className="videoCoverShade" />
        <span className="videoCoverBadge">▶ Video</span>
      </div>
    );
  }
  return (
    <div className="videoCoverBox" aria-label={title}>
      <VideoPosterImage poster={safePoster} title={title} categoryName={categoryName} />
      <div className="videoCoverShade" />
      <span className="videoCoverBadge">▶ Video</span>
    </div>
  );
...
function CartLineMedia({ line, fallbackName }: { line: CartLine; fallbackName: string }) {
  if (line.imageUrl) return <img src={line.imageUrl} alt={line.name} onError={(event) => fallbackForImageElement(event, undefined, fallbackName)} />;
...
export default function StorefrontPage() {
  ...
  const getImage = useCallback((item: Item | null) => {
    if (!item) return '';
    const savedImage = resolveStorageUrl(item.imageUrl);
  ...
  const getPoster = useCallback((item: Item | null) => {
    if (!item) return '';
    const image = resolveStorageUrl(item?.imageUrl);
  ...
  const load = useCallback(async () => {
    ...
    const ownerPromo: PromoRow | null = restaurantRow.promo_enabled === false ? null : {
      id: `restaurant_${restaurantRow.id}_promo`,
      restaurant_id: restaurantRow.id,
      title: restaurantRow.promo_title || `${Number(restaurantRow.promo_percent || 15)}% OFF`,
      description: restaurantRow.promo_text || 'YOUR NEXT ORDER',
      code: restaurantRow.promo_code || 'ORDA15',
      discount_type: 'percent',
      discount_value: Number(restaurantRow.promo_percent || 15),
      active: true,
    };
    ...
    const baseCats: Category[] = ((catRows || []) as CategoryRow[])
      .filter((cat) => itemCategoryIds.includes(cat.id))
      .map((cat, index) => ({ id: cat.id, name: prettyName(cat.name || `Category ${index + 1}`), sortOrder: cat.sort_order ?? index }));

    const missingCategoryRows = itemCategoryIds
      .filter((categoryId) => !baseCats.some((cat) => cat.id === categoryId))
      .map((categoryId, index) => ({ id: categoryId, name: 'Menu', sortOrder: baseCats.length + index }));
    ...
    const mappedItems: Item[] = liveRows.map((row: ItemRow, index: number) => {
      ...
      const videoUrl = resolveStorageUrl(rawVideo);
      ...
      const posterUrl = resolvedImage || fallbackImage(isVideoMenu ? categoryFromName(categoryName, row.name) : categoryName, row.name || 'video');

      const dbGroups = groupList
        .filter((group) => group.item_id === row.id)
        .map((group) => ({
          id: group.id,
          ...
          choices: choiceList
            ...
            .map((choice) => ({ id: choice.id, name: choice.name || 'Choice', priceDelta: Number(choice.price_delta ?? choice.price ?? 0) })),
        })) as Group[];

      const item: Item = {
        id: row.id,
        categoryId: isVideoMenu ? 'VIDEO_MENU' : row.category_id || `uncategorized_${index}`,
        categoryName,
        name: row.name || 'Menu Item',
        description: row.description || '',
        imageUrl: rawImage,
        videoUrl: rawVideo,
        autoImageUrl: resolvedImage || posterUrl,
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`orda-store-${restaurant.id}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'restaurants', filter: `id=eq.${restaurant.id}` }, () => void load())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'menu_items', filter: `restaurant_id=eq.${restaurant.id}` }, () => void load())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'menu_categories', filter: `restaurant_id=eq.${restaurant.id}` }, () => void load())
      .subscribe();
  ...
  const visibleItems = useMemo(() => {
    if (activeCategory === 'VIDEO_MENU') return videoItems;
    if (activeCategory === 'ALL') return [...photoItems, ...videoItems];
    return photoItems.filter((item) => item.categoryId === activeCategory);
  }, [activeCategory, photoItems, videoItems]);

  const activeCategoryName = useMemo(
    () => activeCategory === 'VIDEO_MENU' ? 'Video Menu' : activeCategory === 'ALL' ? visibleItems[0]?.categoryName || categories[0]?.name || 'universal' : categories.find((category) => category.id === activeCategory)?.name || 'universal',
  ...
  const selectedTotal = useMemo(() => {
    ...
    selectedItem.groups.forEach((group) => {
      ...
      group.choices.forEach((choice) => { if (ids.includes(choice.id)) extra += choice.priceDelta; });
    });
    return (selectedItem.basePrice + extra) * qty;
  }, [selectedItem, picked, qty]);
  ...
  function openItem(item: Item) {
    ...
    item.groups.forEach((group) => { start[group.id] = []; });
    setPicked(start);
    setQty(1);
    setSelectedItem(item);
  }

  function toggleChoice(group: Group, choice: Choice) {
    setPicked((current) => {
      const existing = current[group.id] || [];
      if (group.mode === 'single') return { ...current, [group.id]: existing.includes(choice.id) ? [] : [choice.id] };
      return { ...current, [group.id]: existing.includes(choice.id) ? existing.filter((id) => id !== choice.id) : [...existing, choice.id] };
    });
  }

  function canAdd() {
    if (!selectedItem) return false;
    return selectedItem.groups.every((group) => !group.required || (picked[group.id] || []).length > 0);
  }

  function addToCart() {
    if (!selectedItem || !canAdd()) return;
    const selections = selectedItem.groups
      .map((group) => {
        const ids = picked[group.id] || [];
        const selectedChoices = group.choices.filter((choice) => ids.includes(choice.id));
        return {
          groupName: group.name,
          choiceNames: selectedChoices.map((choice) => (choice.priceDelta > 0 ? `${choice.name} +${money(choice.priceDelta)}` : choice.name)),
          priceDelta: selectedChoices.reduce((sum, choice) => sum + choice.priceDelta, 0),
        };
      })
      .filter((selection) => selection.choiceNames.length);

    const unitTotal = selectedItem.basePrice + selections.reduce((sum, selection) => sum + selection.priceDelta, 0);
    const poster = getPoster(selectedItem);
    const video = resolveStorageUrl(selectedItem.videoUrl);
    const mediaType = isVideoMenuItemFromItem(selectedItem) ? 'video' : 'image';
  ...
  function updateCartQty(lineId: string, nextQty: number) {
    ...
    setCart((current) => current.map((line) => (line.id === lineId ? { ...line, quantity: nextQty, total: line.unitTotal * nextQty } : line)));
  }

  function applyPromoByCode(codeValue?: string) {
    const code = String(codeValue || promoInput).trim().toLowerCase();
    if (!code) {
      setAppliedPromo(null);
      setPromoMessage('');
      return;
    }
    const found = promos.find((promo) => promoCodeValue(promo).toLowerCase() === code || promoTitle(promo).toLowerCase() === code);
    if (!found) {
      setAppliedPromo(null);
      setPromoMessage(copy.promoNotFound);
      setCartOpen(true);
      return;
    }
    setAppliedPromo(found);
    setPromoInput(promoCodeValue(found) || promoTitle(found));
    setPromoMessage(`${promoValueText(found)} ${copy.promoApplied}`);
    setCartOpen(true);
  }

  function useReward(reward: RewardRow) {
    setPromoMessage(`${rewardTitle(reward)} selected. Add your items and checkout direct.`);
    setCartOpen(true);
    scrollToMenu();
  }
  ...
  async function checkout() {
    ...
    const checkoutCart = cart.map((line) => ({
      ...
      itemImage: line.imageUrl,
      item_image: line.imageUrl,
      videoUrl: line.videoUrl || '',
      video_url: line.videoUrl || '',
      itemVideo: line.videoUrl || '',
      item_video: line.videoUrl || '',
  ...
  return (
    <main className={`page ${theme} accent-${accent}`}>
      <div className="topNotice">
        <span>ORDA DIRECT</span>
        <nav>
          <button type="button" onClick={scrollToMenu}>{copy.menu}</button>
          <button type="button" onClick={() => document.getElementById('side-promos')?.scrollIntoView({ behavior: 'smooth' })}>{copy.promos}</button>
          <button type="button" onClick={() => document.getElementById('side-rewards')?.scrollIntoView({ behavior: 'smooth' })}>{copy.rewards}</button>
          <button type="button" onClick={() => document.getElementById('hours')?.scrollIntoView({ behavior: 'smooth' })}>{copy.hours}</button>
        </nav>
        <div className="language">
          <button type="button" className={lang === 'en' ? 'on' : ''} onClick={() => setLang('en')}>EN</button>
          <button type="button" className={lang === 'es' ? 'on' : ''} onClick={() => setLang('es')}>ES</button>
          <button type="button" className="cartBtn" onClick={() => setCartOpen(true)}>🛒<b>{cartCount}</b></button>
        </div>
      </div>

      <section className="hero cleanHero">
        {heroVideo && isVideoUrl(heroVideo)
          ? <SmoothVideo className="heroMedia" src={heroVideo} poster={heroImage} />
          : <img className="heroMedia" src={heroImage} alt={storeName} loading="eager" decoding="async" onError={(event) => fallbackForImageElement(event, 'hero', storeName)} />}
        <div className="heroShade" />
        <div className="heroContent cleanHeroContent">
          <div className="heroLockup">
            {logoImage
              ? <img className="heroLogo" src={logoImage} alt={storeName} loading="eager" decoding="async" onError={(event) => fallbackForImageElement(event, 'logo', storeName)} />
              : <div className="heroLogo brandFallback">ORDA</div>}
            <div className="heroText">
              <small>PREMIUM DIRECT STORE</small>
              <h1>{heroTitle}</h1>
              <p>{heroTagline}</p>
            </div>
          </div>
      ...
      <section className="storeInfoBelowHero">
        <div className="infoLines">
          <p>📍 {address}</p>
          <p>☎ {phone || copy.noPhone}</p>
          <p className={openStatus.open ? 'openLine isOpen' : 'openLine isClosed'}><span /> {openStatus.text} <em>•</em> {openStatus.nextText}</p>
        </div>

        <div className="serviceActions">
          {restaurant.pickup_enabled !== false ? <button type="button" className={orderType === 'pickup' ? 'activeService' : ''} onClick={() => setOrderType('pickup')}>🛍 {copy.pickup}</button> : null}
          {restaurant.delivery_enabled ? <button type="button" className={orderType === 'delivery' ? 'activeService' : ''} onClick={() => setOrderType('delivery')}>🚘 {copy.delivery}</button> : null}
          <small>⏱ 30-45 MIN</small>
        </div>

        {socialLinks.length ? (
          <div className="socialStrip" aria-label={copy.followUs}>
            <strong>{copy.followUs}</strong>
            <div>{socialLinks.map((link) => <a key={link.key} className={`socialLink ${link.key}`} href={link.url} target="_blank" rel="noreferrer" aria-label={link.label}><span>{link.icon}</span><b>{link.label}</b></a>)}</div>
          </div>
        ) : null}
      </section>

      {videoItems.length ? (
        <section className="videoMenuSection">
          <div className="sectionHeader">
            <small>ORDA VIDEO MENU</small>
            <h2>{copy.videos}</h2>
          </div>
          <div className="videoRail">
            {videoItems.map((item) => {
              ...
              return (
                <button type="button" key={item.id} className="videoCard" onClick={() => openItem(item)}>
                  {resolveStorageUrl(item.videoUrl) && isVideoUrl(resolveStorageUrl(item.videoUrl))
                    ? <SmoothVideo src={resolveStorageUrl(item.videoUrl)} poster={poster || undefined} limitSeconds={MENU_VIDEO_LIMIT_SECONDS} />
                    : <VideoPosterImage poster={poster} title={item.name} categoryName={item.categoryName} />}
                  <span>▶</span>
      ...
      <section className="layout" id="menu">
        <div className="mainCol">
          <div className="categories">
            <button type="button" className={activeCategory === 'ALL' ? 'active' : ''} onClick={() => setActiveCategory('ALL')}>▦ {copy.allItems}</button>
            {categories.map((cat) => <button type="button" key={cat.id} className={activeCategory === cat.id ? 'active' : ''} onClick={() => setActiveCategory(cat.id)}>{cat.name}</button>)}
            {videoItems.length ? <button type="button" className={activeCategory === 'VIDEO_MENU' ? 'active' : ''} onClick={() => setActiveCategory('VIDEO_MENU')}>▶ {copy.videos}</button> : null}
          </div>

          <h3 className="sectionTitle">{activeCategory === 'VIDEO_MENU' ? copy.videos : activeCategory === 'ALL' ? copy.allItems : categories.find((category) => category.id === activeCategory)?.name}</h3>

          <div className="menuGrid">
            {visibleItems.length ? visibleItems.map((item) => {
              const itemImage = getImage(item);
              const itemVideo = resolveStorageUrl(item.videoUrl);
              const hasVideo = itemVideo && isVideoUrl(itemVideo);
              const poster = getPoster(item);
              return (
                <button type="button" key={item.id} className={hasVideo ? 'itemCard videoItemCard' : 'itemCard'} onClick={() => item.available && openItem(item)}>
                  <div className="imageBox">
                    {hasVideo
                      ? <VideoCover src={itemVideo} poster={poster} title={item.name} categoryName={item.categoryName} />
                      : itemImage
                        ? <img src={itemImage} alt={item.name} loading="lazy" decoding="async" onError={(event) => fallbackForImageElement(event, item.categoryName, item.name)} />
                        : <div className="missingImage" />}
                    {!item.available ? <em>{copy.soldOut}</em> : null}
                  </div>
                  <div className="itemCopy">
                    <h4>{item.name}</h4>
                    <p>{item.description || 'Fresh made item from this ORDA storefront.'}</p>
                    <strong className="itemPrice">{money(item.basePrice)}</strong>
                    <span>⚙ {copy.customize}</span>
                  </div>
                </button>
              );
            }) : <div className="noItems"><strong>{copy.noItems}</strong><span>{copy.noItemsSub}</span></div>}
          </div>
        </div>

        <aside className="sideCol">
          <section className="sideBox mapBox">
            <h3>📍 {copy.location}</h3>
            <p>{address}</p>
            <iframe title="Store map" loading="lazy" src={`https://www.google.com/maps?q=${encodeURIComponent(address)}&output=embed`} />
            <a className="goldFull" href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(address)}`} target="_blank" rel="noreferrer">➤ {copy.directions}</a>
          </section>

          {restaurant.promo_enabled !== false ? (
            <section className="sidePromoVisual" id="side-promos">
              <div className="sidePromoCopy">
                <small>{copy.limitedOffer}</small>
                <h3>{promoBig}</h3>
                <strong>{promoSmall}</strong>
                <p>{promoDesc}</p>
                <button type="button" onClick={() => applyPromoByCode(promoCode)}>{copy.applyCode}</button>
              </div>
              <img src={sidePromoImage} alt={`${activeCategoryName} promo`} loading="lazy" decoding="async" onError={(event) => fallbackForImageElement(event, activeCategoryName, activeCategoryName)} />
            </section>
          ) : null}

          {restaurant.rewards_enabled !== false ? (
            <section className="sideRewardVisual" id="side-rewards">
              <div>
                <small>👑 {featuredReward ? rewardTitle(featuredReward) : 'ORDA REWARDS'}</small>
                <h3>{rewardPoints}</h3>
                <strong>{copy.pointsAvailable}</strong>
                <p>{featuredReward ? rewardText : `${copy.nextReward}: ${rewardText}`}</p>
                <button type="button" onClick={() => (featuredReward ? useReward(featuredReward) : scrollToMenu())}>{copy.viewRewards} →</button>
              </div>
              <img src={sidePromoImage} alt={`${activeCategoryName} rewards`} loading="lazy" decoding="async" onError={(event) => fallbackForImageElement(event, activeCategoryName, activeCategoryName)} />
            </section>
          ) : null}

          <section className="sideBox">
            <h3>🔗 {copy.storeDetails}</h3>
            <p>{restaurant.pickup_enabled !== false ? '✅' : '❌'} {copy.pickupAvailable}</p>
            <p>{restaurant.delivery_enabled ? '✅' : '❌'} {copy.deliveryAvailable}</p>
            <p>🚚 {copy.deliveryFee}: {money(Number(restaurant.delivery_fee || 0))}</p>
            <p>🚚 {copy.deliveryRadius}: {Number(restaurant.delivery_radius || 5)} Miles</p>
            <p>⏱ {copy.minimumOrder}: {money(Number(restaurant.delivery_minimum || 0))}</p>
          </section>

          {socialLinks.length ? (
            <section className="sideBox sideSocialBox">
              <h3>📲 {copy.social}</h3>
              <div className="sideSocialGrid">{socialLinks.map((link) => <a key={link.key} className={`socialLink ${link.key}`} href={link.url} target="_blank" rel="noreferrer" aria-label={link.label}><span>{link.icon}</span><b>{link.label}</b></a>)}</div>
            </section>
          ) : null}

          <section className="sideBox" id="hours">
            <h3>🕒 {copy.hours}</h3>
            {hourRows.map((row) => <div className="hour" key={row.day}><span>{row.day}</span><b>{row.closed ? 'Closed' : `${formatHour(row.open)} - ${formatHour(row.close)}`}</b></div>)}
            <p className={openStatus.open ? 'green statusNow' : 'red statusNow'}>● {openStatus.text} • {openStatus.nextText}</p>
          </section>
        </aside>
      </section>
      ...
      {selectedItem ? (
        <div className="modalBackdrop" onClick={() => setSelectedItem(null)}>
          <section className="modal" onClick={(event) => event.stopPropagation()}>
            <button type="button" className="close" onClick={() => setSelectedItem(null)}>×</button>
            {resolveStorageUrl(selectedItem.videoUrl) && isVideoUrl(resolveStorageUrl(selectedItem.videoUrl))
              ? <SmoothVideo className="modalImg" src={resolveStorageUrl(selectedItem.videoUrl)} poster={getPoster(selectedItem) || undefined} limitSeconds={MENU_VIDEO_LIMIT_SECONDS} controls playWhenVisible={false} />
              : getImage(selectedItem)
                ? <img className="modalImg" src={getImage(selectedItem)} alt={selectedItem.name} onError={(event) => fallbackForImageElement(event, selectedItem.categoryName, selectedItem.name)} />
                : null}

            <div className="modalBody">
              <div className="modalTop">
                <div><h2>{selectedItem.name}</h2><p>{selectedItem.description}</p></div>
                <div className="priceBox">
                  <small>{copy.basePrice}</small><strong>{money(selectedItem.basePrice)}</strong>
                  <small>{copy.finalPrice}</small><b>{money(selectedTotal)}</b>
                </div>
              </div>

              {selectedItem.groups.map((group) => (
                <section className="optionGroup" key={group.id}>
                  <header><h3>{group.name}</h3><span>{group.required ? copy.required : copy.optional}</span></header>
                  <div className="choiceList">
                    {group.choices.map((choice) => {
                      const active = (picked[group.id] || []).includes(choice.id);
                      return <button type="button" key={choice.id} className={active ? 'choice active' : 'choice'} onClick={() => toggleChoice(group, choice)}><span>{choice.name}</span><b>{choice.priceDelta > 0 ? `+${money(choice.priceDelta)}` : '$0.00'}</b><i>{active ? '✓' : ''}</i></button>;
                    })}
                  </div>
                </section>
              ))}
              ...
              <button type="button" className="addBtn" disabled={!canAdd()} onClick={addToCart}>{copy.addToCart} • {money(selectedTotal)}</button>
            </div>
          </section>
        </div>
      ) : null}

      <section className="stickyCart">
        <div className="cartLeft"><span>🛒</span><b>{cartCount}</b></div>
        <div><strong>{cartCount} {copy.items}</strong><p>{copy.cart}</p></div>
        <div className="cartMini">{cart.slice(-3).map((line) => <CartLineMedia key={line.id} line={line} fallbackName={line.name} />)}</div>
        <div className="total"><small>{copy.total}</small><strong>{money(cartTotal)}</strong></div>
        <button type="button" onClick={() => setCartOpen(true)}>{copy.cart} →</button>
      </section>

      {cartOpen ? (
        <div className="cartOverlay" onClick={() => setCartOpen(false)}>
          <aside className="cartPanel" onClick={(event) => event.stopPropagation()}>
            <header><h2>{copy.cart}</h2><button type="button" onClick={() => setCartOpen(false)}>×</button></header>

            <div className="orderTypeBox">
              <small>{copy.orderType}</small>
              <div>
                <button type="button" className={orderType === 'pickup' ? 'active' : ''} disabled={restaurant.pickup_enabled === false} onClick={() => setOrderType('pickup')}>{copy.pickup}</button>
                <button type="button" className={orderType === 'delivery' ? 'active' : ''} disabled={!restaurant.delivery_enabled} onClick={() => setOrderType('delivery')}>{copy.delivery}</button>
              </div>
            </div>

            {!cart.length ? <p className="empty">{copy.empty}</p> : null}

            {cart.map((line) => (
              <div className="cartLine" key={line.id}>
                <CartLineMedia line={line} fallbackName={line.name} />
                <div className="cartLineBody">
                  <div className="cartLineTop"><h4>{line.name}</h4><button className="removeLine" type="button" onClick={() => removeCartLine(line.id)}>{copy.remove}</button></div>
                  {line.selections.map((selection, index) => <p key={index}>{selection.groupName}: {selection.choiceNames.join(', ')}</p>)}
                  <div className="cartEditRow">
                    <div className="cartQty"><button type="button" onClick={() => updateCartQty(line.id, line.quantity - 1)}>−</button><span>{line.quantity}</span><button type="button" onClick={() => updateCartQty(line.id, line.quantity + 1)}>+</button></div>
            ...
            <div className="promoApply">
              <div><input value={promoInput} onChange={(event) => setPromoInput(event.target.value)} placeholder={copy.promoCode} /><button type="button" onClick={() => applyPromoByCode()}>{copy.apply}</button></div>
              {promoMessage ? <p>{promoMessage}</p> : null}
            </div>

            <div className="cartTotals">
              <div><span>{copy.subtotal}</span><b>{money(cartSubtotal)}</b></div>
              {deliveryFee > 0 ? <div><span>{copy.deliveryFee}</span><b>{money(deliveryFee)}</b></div> : null}
              {promoDiscount > 0 ? <div className="discountLine"><span>{copy.discount}</span><b>-{money(promoDiscount)}</b></div> : null}
              <div className="grandTotal"><span>{copy.total}</span><b>{money(cartTotal)}</b></div>
            </div>

            <button type="button" className="addBtn" onClick={checkout} disabled={!cart.length || checkingOut}>{checkingOut ? copy.working : `${copy.checkout} • ${money(cartTotal)}`}</button>
          </aside>
        </div>
      ) : null}
