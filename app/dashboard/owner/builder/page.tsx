...
const copy = {
  en: {
    ...
    subtitle: 'Easily update your store information, upload images, menu items, and connect payments.',
    ...
    storeNamePlaceholder: 'The Cultured Soul Kitchen',
    phonePlaceholder: '(555) 123-4567',
    addressPlaceholder: '123 Main St, City, State',
    hoursPlaceholder: 'Mon-Sat 10am - 8pm',
    itemNamePlaceholder: 'Oxtail Dinner',
    pricePlaceholder: '20.00',
    descriptionPlaceholder: 'Tender, slow-simmered oxtails served over rice.',
    remove: 'Remove',
  ...
  es: {
    ...
    subtitle: 'Actualiza fácilmente la información de tu tienda, sube imágenes, agrega productos y conecta pagos.',
    ...
    itemNamePlaceholder: 'Cena de rabo',
    pricePlaceholder: '20.00',
    descriptionPlaceholder: 'Rabo tierno servido sobre arroz.',
    remove: 'Eliminar',
...
export default function BuilderPage() {
    ...
    const loadBuilder = async () => {
      try {
        ...
        let { data: restaurantData, error: restaurantError } = await supabase
          .from('restaurants')
          .select(
            'id, owner_id, name, slug, phone, address, hours, hero_url, logo_url, owner_email, stripe_account_id, stripe_connected, stripe_charges_enabled, stripe_payouts_enabled'
          )
          .eq('owner_id', user.id)
          .maybeSingle();
        ...
        if (restaurant && mounted) {
          ...
          setStripeAccountId(restaurant.stripe_account_id ?? '');
          setStripeConnected(Boolean(restaurant.stripe_connected));
          setStripeChargesEnabled(Boolean(restaurant.stripe_charges_enabled));
          setStripePayoutsEnabled(Boolean(restaurant.stripe_payouts_enabled));
        }
  ...
  const handleStripeConnect = async () => {
    try {
      ...
      alert('Stripe connection link created.');
    } catch (error: any) {
      alert(error?.message || t.builderFailed);
    } finally {
  ...
  const handleStripeRefresh = async () => {
    try {
      ...
      setStripeConnected(Boolean(data?.stripe_connected));
    ...
    <main className="page">
      <form onSubmit={handleSave} className="builderWrap">
        ...
        <section className="sectionCard" id="images-section">
          ...
          <button
            type="button"
            className="bannerUploadCard"
            onClick={() => heroInputRef.current?.click()}
          >
            {heroUrl ? (
              <>
                <div className="bannerPreviewWrap">
                  <img src={heroUrl} alt="Hero banner" className="bannerPreview" />
                </div>
                <div className="changeImageBar">
                  {heroUploading ? t.loading : t.changeImage}
                </div>
              </>
            ) : (
              <>
          ...
          <div className="paymentGrid">
            <div className="paymentStat">
              ...
              <div className="paymentValue">
                {stripeAccountId ? stripeAccountId : stripeConnected ? t.connected : t.incomplete}
              </div>
            </div>
            <div className="paymentStat">
              <div className="paymentLabel">{t.charges}</div>
              <div className="paymentValue">
                {stripeChargesEnabled ? t.connected : t.incomplete}
              </div>
            </div>
            <div className="paymentStat">
              <div className="paymentLabel">{t.payouts}</div>
              <div className="paymentValue">
                {stripePayoutsEnabled ? t.connected : t.incomplete}
              </div>
            </div>
          </div>
