...
function getStatusClass(status?: string | null) {
  ...
  if (s.includes('prepar') || s.includes('new') || s.includes('complete')) return 'status-green';
...
function filterOrders(list: OrderRow[], filter: OrderFilter) {
  ...
  if (filter === 'YELLOW') {
    return list.filter((item) => getStatusClass(item.status) === 'status-yellow');
  }
  if (filter === 'GREEN') {
    return list.filter((item) => getStatusClass(item.status) === 'status-green');
  }
  return list;
}
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    async function loadDashboard() {
      ...
      const restaurantByOwnerResult = await supabase
        .from('restaurants')
        .select('*')
        .eq('owner_id', user.id)
        .maybeSingle();

      let currentRestaurant = restaurantByOwnerResult.data as RestaurantRecord | null;

      if (!currentRestaurant) {
        const restaurantByUserResult = await supabase
          .from('restaurants')
          .select('*')
          .eq('user_id', user.id)
          .maybeSingle();

        currentRestaurant = restaurantByUserResult.data as RestaurantRecord | null;
      }
      ...
      if (currentRestaurant?.id) {
        ...
        currentMenuCount = (menuResult.data || []).length;
        ...
        currentOrders = (ordersResult.data || []) as OrderRow[];
      }
